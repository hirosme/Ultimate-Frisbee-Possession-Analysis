...
colnames(arr_melt) = c('Y_0','X_0','Score_Prob')
arr_melt = arr_melt[,c(2,1,3)]
arr_melt$Y_0 = arr_melt$Y_0*(FIELD_HEIGHT/NROWS)
arr_melt$Y_0 = 100 - arr_melt$Y_0

## Adding turnover cost column

turn_cost = function(x,y,df) {
  ...
  return(df[df$X_0==x & arr_melt$Y_0==y,3])
}

arr_melt$Turnover_cost = apply(arr_melt[,c(1,2)],1,function(x){
                          x = as.numeric(x)
                          turn_cost(x[1],x[2],arr_melt)}
                         )
...
## Score % heatmap
p = ggplot(arr_melt,aes(x = X_0, y = Y_0, z=Score_Prob, fill = Score_Prob)) + 
  geom_tile() + 
